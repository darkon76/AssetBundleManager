...
public class AssetBundleCoordinator
{
    ...
    public T Instatiate<T>(AssetReference reference, Transform parent = null) where T : Object
    {
        ...
        return Object.Instantiate<T>(assetRef, parent);
    }
    ...
    public T GetAsset<T>(AssetReference reference)  where T: Object
    {
        if (!_loadedBundles.TryGetValue(reference.Bundle, out var bundle))
        {
            ...
            if (bundle == null)
            {
                Debug.LogError($"{nameof(AssetBundleCoordinator)} - Instantiate bundle wasn't found: {reference.Bundle}");
                return null;
            }
